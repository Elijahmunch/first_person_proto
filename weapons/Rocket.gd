...
var explosions = preload("res://weapons/Explosion.tscn")
...
func explode():
	...
	var explosions_inst = explosions.instance()
	get_tree().get_root().add_child(explosions_inst)
	explosions_inst.global_transform.origin = global_transform.origin
	explosions_inst.explode()
