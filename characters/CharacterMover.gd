...
func _physics_process(delta):
	...
	if !ignore_rotation:
		cur_move_vec = cur_move_vec.rotated(Vector3(0,1,0), body_to_move.rotation.y)
	velocity += move_accel * cur_move_vec - velocity * Vector3(drag, 0, drag) + gravity * Vector3(0,-1,0) * delta
	velocity = body_to_move.move_and_slide_with_snap(velocity, snap_vec, Vector3(0,1,0))
